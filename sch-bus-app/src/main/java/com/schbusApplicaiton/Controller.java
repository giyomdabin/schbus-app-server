...
public class Controller {
    ...
    @PostMapping("/selected_seat") //좌석 선택 -> 카카오페이 결제
    public ResponseEntity<?> selectedSeat() {

        List<Reserve> existingReservations = reserveRepository.findAll();
        ...
        for(Reserve reserve : existingReservations) {
            ReserveDto reserves = new ReserveDto();
            reserves.setUser(reserve.getUser().toString());
            reserves.setBus(reserve.getBus().toString());
            reserves.setDate(reserve.getDate().toString());
            reserves.setSeat(reserve.getSeat().toString());

            reservationInfo.add(reserves);
        }
    ...
    public ResponseEntity<?> selectSeat(@RequestBody ReserveDto reserveDto, HttpSession session) {
        ...
        ResponseEntity.ok(reservationInfo);*/
